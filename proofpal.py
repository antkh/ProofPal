...
Original file is located at
    https://colab.research.google.com/drive/1Ag6dnCIB2P703Mhq_7UD-NrJ8o4-eg0P
"""
...
def encode_state(state): # encodes state to a tuple, so that we can use it as index for the q-table (hash table)
...
def get_qtable(env_initial, n_Pred, n_Vars_Uni, n_Vars_Inst, n_Funs, n_Statements):
    ...
    q_table = defaultdict(lambda: [0] * (num_actions)) # initialize all 0s



    def choose_action(state_idx, eps): # using epsilon-greedy policy
      ...
      while not done and iters < MAX_ITER:
          ...
          if action < N*(N-1): # extend
              ...
              if env.extend(i, j):
                  ...
                  reward = -1 # always remove -1, cost of step
              else:
                  reward = -2 # remove -2, for choosing indices which are irrelevant
              ...
              if env.contradict(i, j): # we have found a contradiction, the proof is complete
                  done  = True
                  reward = 100
              else:
                  reward = -2
              ...
              if env.instantiate(i, uni_var, inst):
                  next_state = env.state
                  next_state_idx = encode_state(next_state)
                  reward = -1 # for extra step
              else:
                  reward = -2 # for illegal choice of parameters to function



          if iters == MAX_ITER:
              reward -= 100 # penalty for taking too long, and we stop the proof and repeat

          # update q-values
          max_next_q = max(q_table[tuple(next_state_idx)])
          q_table[state_idx][action] += ALPHA * (reward + GAMMA * max_next_q - q_table[state_idx][action]) # Bellman equation
...
env_initial = [[[-1, 1, 0], [2, 1, 1]], [[-2, 1, 0], [3, 1, 1]], [[1, -1, 0]], [[-3, 1, 0]]]

getProof(env_initial, 3, 1, 1, 2, 4)